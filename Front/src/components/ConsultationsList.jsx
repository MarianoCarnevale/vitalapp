      ...
      <section className="z-10">
         <div className="flex flex-col gap-2 items-center p-4 bg-white w-full  border-primary rounded-3xl">
          ...
          { results.filter(result => result.is_pending === 1).length > 0 &&
            results.filter(result => result.is_pending === 1)
              .map((result) => {
                return (
                  <li
                  ...
                  >
                    ...
                    <p className={`p-1 w-20 rounded-xl text-white ${getStatusClass(result.severity)}`}>
                      {result.severity}
                    </p>
                    <p>
          ...
          { consultations.filter(consultation => consultation.is_pending === 0).length > 0 && <ul className="w-full flex flex-col justify-center gap-5 bg-white p-5  border-white rounded-3xl min-h-72 max-h-96 overflow-auto hide-scrollbar shadow-lg">
        ...
        />
            ...
            {results.filter(consultation => consultation.is_pending === 0)
          .map((consultation) => {
            return (
              <li
                className="flex justify-between items-center  gap-5 shadow-xl p-4 text-primary font-bold rounded-3xl"
