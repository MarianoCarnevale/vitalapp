...
export const selectAllResponsesByConsultationModel = async (consultation_id) => {
  try {
    ...
    const responses = await pool.query(
      `SELECT U.user_id, U.first_name, U.role, R.consultation_id, R.content, AVG(RT.rating_value) AS rating FROM responses R
        INNER JOIN users U ON R.user_id = U.user_id
        INNER JOIN ratings RT ON RT.response_id = R.response_id
        WHERE consultation_id = ?
      GROUP BY U.user_id, R.response_id, consultation_id
      `,
      [consultation_id]
    );
