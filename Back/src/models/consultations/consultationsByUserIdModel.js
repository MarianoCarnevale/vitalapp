...
export const consultationsByUserIdModel = async (filter) => {
  try {
    ...
    SELECT 
    C.consultation_id, doctor.doctor_user_id,
    ...
    C.severity, C.created_at
  FROM consultations C
  LEFT JOIN (
      SELECT D.user_id as doctor_user_id, D.doctor_id  as doctor_id, U.first_name AS doctor_first_name, U.last_name AS doctor_last_name,DS.discipline_name AS speciality
      FROM doctors D 
